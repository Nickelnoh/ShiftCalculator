...
function normalizeState() {
    currentYear = clamp(parseInt(localStorage.getItem(STORAGE_KEYS.year), 10) || DEFAULT_YEAR, 2000, 2100);
    currentCellHeight = clamp(parseInt(localStorage.getItem(STORAGE_KEYS.cellHeight), 10) || DEFAULT_CELL_HEIGHT, CELL_HEIGHT_MIN, CELL_HEIGHT_MAX);

    const rawGraphs = safeParseJSON(localStorage.getItem(STORAGE_KEYS.graphs), []);
    graphs = Array.isArray(rawGraphs) && rawGraphs.length
    ...
    absences = Array.isArray(rawAbsences)
